  ...
  return (
    <div className="event-card">
      ...
      <div className="event-card-content">
        ...
        {/* Conditional Buttons */}
        <button
          onClick={() => userRole === 'admin' || event.status === 'upcoming' ? onViewDetails(event.id) : onAttend(event.id)}
          className={`button ${userRole === 'admin' || event.status === 'upcoming' ? 'button-primary' : 'button-success'} d-flex align-items-center justify-content-center`}
          style={{ width: '100%' }}
        >
          {userRole === 'admin' || event.status === 'upcoming' ? 'View Details' : 'Attend'}
        </button>
      </div>
    </div>
  );
